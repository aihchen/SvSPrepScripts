...
function scheduleBasedOnSort(sheet) {
  ...
  const availabilities = sheet.getRange("L7:L54").getValues().flat(); // Cleaned availability
  const startTimes = sheet.getRange("M7:M54").getValues().flat().map(normalizeTime); // Time intervals
  const contributions = sheet.getRange("E7:E54").getValues().flat();  // Sort item - refined crystal, crystal shard, troop promotion
  const targetColumn = 14; // Column N — Sorted Item schedule
...
function scheduleBasedOnLimitedAvailability(sheet) {
  ...
  const availabilities = sheet.getRange("L7:L54").getValues().flat(); // Cleaned availability
  const startTimes = sheet.getRange("M7:M54").getValues().flat().map(normalizeTime); // Time intervals
  const targetColumn = 15; // Column O — Limited Availability schedule
  ...
  // Highlight blank slots in column O
...
function scheduleAllSlots(sheet) {
  ...
  const availabilities = sheet.getRange("L7:L54").getValues().flat();
  const startTimes = sheet.getRange("M7:M54").getValues().flat().map(normalizeTime);
  ...
  // Highlight blank slots in column P (target column)
  const targetRange = sheet.getRange(7, targetColumn, startTimes.length);
  const values = targetRange.getValues();
...
function scheduleBasedOnSpeedups(sheet) {
  ...
  const availabilities = sheet.getRange(START_ROW, 12, RANGE_SIZE).getValues().flat(); // Column L
  const startTimes = sheet.getRange(START_ROW, 13, RANGE_SIZE).getValues().flat().map(normalizeTime); // Column M
  const speedups = sheet.getRange(START_ROW, 8, RANGE_SIZE).getValues().flat();        // Column H

  const targetColumn = 17; // Column Q
  const assignedPlayers = new Set();
  const seenNames = new Set();
  const playerPool = [];
  ...
  // Highlight unfilled slots in column Q
...
function highlightDuplicateNames(sheet) {
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    if (name && nameCounts[name] > 1) {
      // Highlight cells A–L for the duplicate row
      sheet.getRange(i + 7, 1, 1, 12).setBackground("grey");
    }
  }
}

// Main function to clear old results and run all 4 strategies
function Schedule() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getActiveSheet();

  // Clear previous assignments and highlights
  sheet.getRange("N7:Q54").clearContent();      // Columns N, O, P, Q
  sheet.getRange("B7:B54").setBackground(null); // Reset name highlights
  sheet.getRange("N7:Q54").setBackground(null); // Reset start time highlights

  // Run all 4 scheduling strategies
